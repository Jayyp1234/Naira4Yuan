							...
							<div className="flex flex-col gap-y-3.5" key={index}>
								...
								<ul className="flex flex-col gap-y-3">
									{each.list.map((e, i) => {
										return (
											<li key={`${index}${i}`}>
												<Link to={e.to} className={`flex items-center gap-3.5 flex-grow hover:text-slate-800 text-slate-600 ${commonLinkStyle}`}>
													...
													<div className="flex items-center justify-between flex-grow gap-3">
														<aside className="text-start">
															...
															{e.description && <span className="text-gray-500 text-sm leading-tight">{e.description}</span>}
														</aside>
														<IconWrapper>
